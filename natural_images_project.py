...
# 2. Dynamic Model Downloader & Loader (Cached so it only runs once)
@st.cache_resource
def load_champion_model():
    model_path = 'best_natural_images_model.keras'
    
    # Check if the model file is already downloaded in the environment
    if not os.path.exists(model_path):
        # We create a status container so the user sees the progress bar
        ...
        try:
            ...
            status_text.empty() # Clear the download text once finished
