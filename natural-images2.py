...
# -------------------------------------------------------
# 1. Custom Dense layer that ignores 'quantization_config'
#    and also handles the 'dtype' serialization issue
# -------------------------------------------------------
...
class CustomDense(Dense):
    ...
    def from_config(cls, config):
        ...
        # If dtype is a dict (DTypePolicy serialization), convert to string
        if 'dtype' in config and isinstance(config['dtype'], dict):
            # Extract the actual dtype name (e.g., 'float32')
...
def load_champion_model():
    ...
    # Register our custom Dense globally, then load
    with tf.keras.utils.custom_object_scope({'Dense': CustomDense}):
        # safe_mode=False allows loading even if some config keys are unknown
