...
# -------------------------------------------------------
# 1. Monkey-Patch Keras Dense layer to drop 'quantization_config'
#    This safely fixes the deserialization error globally.
# -------------------------------------------------------
def patch_dense_layer():
    # Target both standalone keras and tensorflow's bundled keras layers
    for dense_class in [keras.layers.Dense, tf.keras.layers.Dense]:
        if hasattr(dense_class, '__init__'):
            original_init = dense_class.__init__
            def patched_init(self, *args, **kwargs):
                # Intercept and remove the troublesome parameter if present
                kwargs.pop('quantization_config', None)
                return original_init(self, *args, **kwargs)
            dense_class.__init__ = patched_init
...
def load_champion_model():
    ...
    if not os.path.exists(model_path):
        ...
        # Keeping your unique Google Drive file ID intact
    ...
    # Load the model cleanly (safe_mode=False ignores strict metadata mismatches)
    try:
